...
| --------------------------------- | ------------------------------------------------------- | ------------------------------------------------ |
| `com.machiav3lli.fdroid.hmos`         | HMOS bundle name (reverse-DNS, ends in `.hmos`)         | `com.beemdevelopment.aegis.hmos`                 |
| `NeoStore`              | Vendor / publisher name (usually app name)              | `Aegis`                                          |
| `1`        | Integer version code                                    | `1`                                              |
| `1.0.0`        | Semver version string                                   | `1.0.0`                                          |
| `NeoStore`           | Display name shown on the launcher                      | `Aegis`                                          |
| `neostore`      | Lowercase, no-space identifier for `oh-package.json5`   | `aegis`                                          |
| `NeoStore F-Droid client for HarmonyOS` | One-line project description                            | `Aegis 2FA authenticator for HarmonyOS`          |
| `NeoStore main module`         | Description for `entry/src/main/module.json5` (module)  | `Aegis main module`                              |
| `NeoStore main entry ability`        | Description for `entry/src/main/module.json5` (ability) | `Aegis main entry ability`                       |
...
find "${DEST}" -type f \( -name '*.json5' -o -name '*.json' -o -name '*.ts' -o -name '*.ets' -o -name '*.md' \) \
  -exec sed -i \
    -e 's|com.machiav3lli.fdroid.hmos|com.beemdevelopment.aegis.hmos|g' \
    -e 's|NeoStore|Aegis|g' \
    -e 's|1|1|g' \
    -e 's|1.0.0|1.0.0|g' \
    -e 's|NeoStore|Aegis|g' \
    -e 's|neostore|aegis|g' \
    -e 's|NeoStore F-Droid client for HarmonyOS|Aegis 2FA authenticator for HarmonyOS|g' \
    -e 's|NeoStore main module|Aegis main module|g' \
    -e 's|NeoStore main entry ability|Aegis main entry ability|g' \
    {} +
```
...
$map = @{
  'com.machiav3lli.fdroid.hmos'         = 'com.beemdevelopment.aegis.hmos'
  'NeoStore'              = 'Aegis'
  '1'        = '1'
  '1.0.0'        = '1.0.0'
  'NeoStore'           = 'Aegis'
  'neostore'      = 'aegis'
  'NeoStore F-Droid client for HarmonyOS' = 'Aegis 2FA authenticator for HarmonyOS'
  'NeoStore main module'         = 'Aegis main module'
  'NeoStore main entry ability'        = 'Aegis main entry ability'
}
